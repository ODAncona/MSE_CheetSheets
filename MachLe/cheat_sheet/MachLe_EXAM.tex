    ...
    \begin{tikzpicture}
        \node [mybox] (box){%
         \begin{minipage}{0.247\textwidth}
            ...
            \textbf{G = 0.5} : maximum value of impurity, classes are balanced in the set.\\
            \textbf{G = 0} : minimum value of impurity, all the values belong to a single class.\\

            \textbf{Gini Impurity of a Split}:\\
            $ Gini_{split} = \displaystyle\sum_{i=1}^{n} \dfrac{N_i}{N} G(X_i) $\\
            $ Gini_{gain} = Gini_{set} - Gini_{split} $\\
