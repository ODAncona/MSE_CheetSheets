...
\begin{tikzpicture}
    \node [mybox] (box){%
        \begin{minipage}{0.247\textwidth}
            ...
            High-dimensional spaces may obscure clustering due to sparsity and equidistance issues.\\

            \nc{PARTITIONAL CLUSTERING}
            Hierarchical (nested clusters) vs Partitional (disjoint clusters).\\

            \nc{Cluster Evaluation}
            Includes measures like Entropy, Purity, Rand Index, Mutual Information, and Conditional Entropy.\\
    
            \nc{Distance Measures}
            Important for determining cluster membership: Euclidean, Manhattan, Minkowski, Chebyshev, Hamming, Cosine, Jaccard.\\
    
            \nc{Affinity Propagation}
            \textbf{Algorithm}:
            \begin{enumerate}[noitemsep]
                \item Initialize similarity and availability matrices.
                \item Update responsibility and availability matrices iteratively.
                \item Identify exemplars and form clusters.
            \end{enumerate}
            \textbf{(+)} Automatically determines the number of clusters; can find exemplars.\\
            \textbf{(-)} Computationally expensive; not suitable for very large datasets.\\
        \end{minipage}
    };
    \node[fancytitle, right=10pt] at (box.north west) {Clustering};
\end{tikzpicture}
